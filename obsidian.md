...
Capture highlights from the web.
...
Add a command to view a single document leaf in a fullscreen focus mode.
...
Update your Discord Status to show your friends what you are working on.
...
Text-to-speech adds sound, audio, and speech to your notes, letting them talk in your workspace, mobile-friendly, perfect for learning or reinforcing ideas as you listen hands-free in an audiobook-like experience.
...
Word Sprint for your writing projects like NaNoWriMo.
...
Translate selected text into more than 25 languages with DeepL.
...
Add edges between notes in Canvas based on their links.
...
Official D2 plugin. D2 is a modern diagram scripting language that turns text to diagrams.
...
View the differences between two files with merge options.
...
Create and save snaps or video recordings.
...
Show the current typing speed in the status bar.
...
Trash orphaned files and attachments.
...
Quickly copy Markdown as LaTeX, with citations.
...
Define commands for creating planning documents and moving tasks between them.

Focused on the current line, others faded like a ghost!

更符合中国习惯的日历，可以显示农历、节气、节假日、调休信息，支持月视图和年视图切换，支持关联创建周期性笔记。

Hide information until you want to reveal it.
...
Add multiple other programs to open notes with.
...
Choose an emotion from a list to insert into a note.
...
Embed Plotly charts in your notes.
...
Integrate Habitica tasks and stats.

Add asciimath support.

Search links without notes in your vault.
...
Move completed to-do items to a separate completed area.
...
Display the page linked 2hop ahead.
...
Enable alt+↑ and alt+↓ keyboard navigation.
...
Enables flexible conversion of note views into various layout (Closed source).
...
Use the familiar @ notation to cross link to people files in a people folder.
...
Commands to publish notes to Quip.com.
...
Intelligently transcribe and categorize your voice notes.
...
Automatically add the current time to new bullet lines.
...
Import and convert your bookmarks from Floccus.
...
Add a status bar indicator showing how long your vault has been open.
...
Automatically resize the node when the content changes.
...
Templating powered by Nunjucks.

Upload images to hosting platforms, or files to GitHub and embed them as Markdown file/image links. 
...
Display relative line numbers in the editor's gutter.
...
Take a stock symbol and returns a callout block with the latest stock information.
...
Easily generate beautiful gradient images from text and share them on social media.
...
Save messages into vault which sending through WeChat/HTTP API/Email
...
Pin frequently-used notes on Ribbon actions (left sidebar)
...
Show your favorite quote in the status bar.
...
Makie inserting external links easier to your notes.
...
Take attendance directly inside of your notes.
...
Find semantically similar notes using local AI. Works on desktop and mobile without external servers.
...
A bridge from your vault to Docusaurus.
...
Render NPC and adversary statblocks for The One Ring 2E roleplaying game.
...
Streamline your stat/habit tracking process by automatically identifying and prefixing unfilled stats, making them easier to spot and fill out.
...
Open URLs based on a permalink or slug in the note properties. Useful with static site generator such as Jekyll, Hugo, Eleventy, etc.
...
Enable evaluating/executing DMNs.

Upload images, videos, audio, PDFs, and other files to Minio OSS.

Build football lineups.

Publish a file to a given POST API.

Makes code blocks collapsible in preview mode as well as enabling scroll-able code blocks.
...
Official HamsterBase integration.
...
Integrate .lhs files into your PKM.

Enable repeated calls of a command using defined or per-invocation values.
...
Automatically make all inline math \displaystyle.

Grab data from IVRE and bring it into your notes.

Allows to insert multiple attachments at a time.

Import notes from a Tolino E-Reader.

Embed and link local files outside your vault with relative paths for cross-device and multi-platform compatibility.

Rotate opened files to the left or right with a specified interval.

Basic support for Markdoc files.

Evaluate Clojure(Script) code blocks in Babashka.

Evaluate, solve and much more within LaTeX blocks using Sympy.

Read any ArchWiki page.

Sync your Readavocado highlights.
...
Create, manage and display Laws of Form expressions like ((a)) (b) = a (b).
...
Pull stories, bugs, chores from Pivotal Tracker.
...
Add a leaf ID parameter to the URI protocol for switching between open tabs with Rofi. A sample Rofi script is included.
...
Upload images from your clipboard to qiniu.com and embed uploaded image to your note.

Enhances Note composer core plugin.

Render APL syntax.

Convert a note to Slack-compliant Markdown.
...
An easy way to collapse the sidebars without going fullscreen.

Publish notes to any write.as blogs you own.
...
Bridge your note-taking and Todoist task management workflows with contextual connections. Seamlessly integrate with Dataview and Tasks plugins.

Track task completion with a visual progress bar in your sidebar, auto update column Kanban

Create feeds of topic-specific bullet points.
...
Send notifications to Google Chat Webhook based on due dates in tasks.
...
Publish your notes in one click.
...
Map timing onto a storyclock. Inspired by Plot Devices Storyclock.
...
Push notes to xlog.app.

View your RescueTime data.
...
Official ReClipped integration.
...
Create single file archive/trash and merge(archive) old notes with it.

Helps you find movies and create notes.

A REPL to digest your thoughts

Manage your Contentful content.

Create recalls from your notes.
...
Add a no-op command to disable hotkeys.
...
Adds a nice completion sound to checkboxes when ticked off
...
List external links on the right side panel.
...
Create all unresolvered links with a single click.
...
Converts Bible texts to JW Library links
...
Copy Obsidian Publish URL to the clipboard.
...
Collects all images from current note and saves them to a new folder
...
Encrypt partial text or complete notes using GPG technology, it is compatible with security keys such as YubiKey or traditional GPG encryption methods
...
Helps you to quickly underline word translations and insert word cards in your notes.

Set custom colors for files and folders.

Manage tag groups and quickly insert tags.

Split a note into individual notes based on a delimiter.

Save Are.na blocks as notes.
...
Copy the contents of files or selections within to the clipboard, without internal links.
...
Context menu to convert image format in reading view.

Sync notes with Confluence
...
Supercharged search across backlinks and relations in your notes

 manage, organize, link and interact with your contacts. structured md files according vCard (VCF) standards
...
Alt-click on codeblocks to copy its data to the clipboard
...
Create daily notes with a specified template according to the described settings.
...
Send the path of the active file to locally installed ManicTime client.
...
Web3, IPFS, and Desci integrations.

Manage your daily tasks as 'ROUTINES' and organize your everyday life more effectively.

MathLive input in editor mode

Helpful commands for starting and working in your vault with the PARA method.

This plugin allows you to transfer LaTeX equations generated by GPT.

Flow through dynamically-scoped collapsible tree views of your vault's semantic and logical architectures.

Post selected text to OMG.lol statuslog.
...
Render Markdown inside the outline view.
...
Support new line order list like 1、 or (1)., etc.
...
This simplifies creating and visualizing timelines from text using dates and metadata.
...
Keep top heading in note synced with file name.
...
Copy and paste formatting attributes (size and color) between canvas elements, similar to the format painter in Word.
...
Render small latex snippets in your notes as SVG and PNG files.
...
Creates searchable metadata notes for Canvas files, PDFs, and images, enhancing graph view visibility and plugin compatibility.
...
Integrate OpenAPI specification management with features for version control, visualization, editing, and easy navigation of API specs.
...
Renders an mapbox location image based on given coordinates.
...
Use the Pinyin tones without having to install extra keyboard layouts.

Prevents any editing, only letting you write ahead.

View .tif(f) files by generating duplicates in form of .tif(f).png

Rollover unchecked todo items from the previous weekly note
...
Specialised graph visualisation for Zettelkasten note hierarchies
...
Extract and download any external files from links to local files
...
Provides a dynamic overview of your vault or folders in the format of a codeblock.
...
Improve copy-paste from PDFs by cleaning newlines and hyphens.
...
Covert text to natural voice audio, locally - Listen Up!
...
Type anything and get in zen mode instantly, no more distraction between you and your thoughts.
...
Add comments to your notes to facilitate collaberation
...
Track news posts from vk.com groups.
...
Verovio Music Renderer displays musical notation from various formats (MEI, MusicXML, abc) and plays it back.
...
Quickly and easily publish your notes to the web with Orion Publish.
...
Open the bible in your preferred translation in a split-screen view within your workspace
...
Synchronize documents between your vault and MrDoc.

Enrich your notes with information from VirusTotal.

Parses the tags in your Daily Notes and create interactive charts in a dashboard view.
...
Get MathJax syntax suggestions from natural language.
...
World building toolset with OnlyWorlds integration.
...
Converts JW Library and Bible references to actual links in JW Library.
...
Allows pasting images from the clipboard into frontmatter properties in live preview.
...
Show draft status in the file explorer.
...
Generate Anki flashcards in multiple ways.
...
Create temporary one-off session notes.
...
Nextcloud breaks Wiki-links. This fixes them.
...
Copy the contents of files and folders in a structured JSON format.
...
Recursively copies all Markdown files in a folder, concatenates them, and copies them into the clipboard.
...
Daily Task Auto Generator can helps you automatically generate daily task files with customizable templates.
...
Instantly creates a TickTick task from the current block of text using a keyboard shortcut. Task includes a copy of the text, and the URI to that exact block of text in your notes. Requires Advanced URI plugin.
...
增强发布插件，支持图片时自动存储、HTML预览和发布到微信公众号等内容平台。
...
View and edit Wardley Maps directly within your notes using the popular Online Wardley Maps (OWM) format.
...
Convert URIs to internal links.

Store images and other attachments on the vault

Plugin to filter and review notes
...
Syncs your watched Trakt.tv shows to a file.
...
Mark and visualize the countries you've visited on an interactive world map.
...
Fast upload images from your clipboard by gopic
...
Convert pinyin to Chinese and LaTeX

Easily create simple tests

Converts raw text timecodes into clickable URLs if a note contains a link to a video
...
Display cards for movies, TV shows, books, and music.
...
The most amazing documentation assistant
...
Restore the link opening method to Ctrl + Left Click.

Import and consolidate highlights from different reading sources. Supports reconciling books across reading sources.

Redirect pasted text into a separate note, and embed it.

Manage projects according to an ontology.

Instantly send a note, to other users so they can import them into their vault via URL.
...
Streamline Japanese note-taking with romaji to kana conversion
...
Easily select text in backticks.
...
Creative writing prompts that can be synced offline.
...
Generate tweets automatically from all the notes you took
...
Display your entire life in weeks, with weekly notes integration.
...
Suggest notes as selected by user.

Support Tengwar (The Elvish Letters).

Fetches current prayer times using the Aladhan API and saves them to a note.
...
Makes text go from left-to-right, bottom-to-top.

disguise/obscure/hide the markdown content in a very simple way.
...
Helps with formatting and citing quotes.
...
Automatically imports recipes collected using Cooksync.
...
Toggle Unlinked mentions ON or OFF within backlinks and Outgoing links panels
...
Adds commands to quickly add entries to INBOX and TIMELINE sections of notes

Render mahjong tiles (riichi mahjong) using MPSZ notation
...
Generate HTTP Links to your vault files for universal URL recognition
...
Update excluded files from the file explorer.
...
Simple auto-completion for LaTeX. As simple as typing '\'.
...
Allows you to post to weblogs.lol, status.lol, some.pics and paste.lol.
...
AI-powered nutrition tracking with food image analysis and daily logging.

Provides an intuitive menu for quickly changing checkbox styles.

Official plugin to interact with Magiedit's API
...
Study and explore The Spirit's Book with search, random questions, and favorites.
...
Add comment on the part of sentence and refer in comment view.
